...
fun DashboardScreen(
    ...
    Box(
        ...
        LazyColumn(
            ...
            item(key = "v2tt-controls") {
                val profile = uiState.profiles.find { it.id == uiState.selectedProfileId }
                io.nekohasekai.sfa.v2tt.V2ttControls(
                    profile = profile,
                    status = serviceStatus,
                    onImport = { onOpenNewProfile(NewProfileArgs(importName = "V2TT", importUrl = "")) },
                    onRefresh = { profile?.let(viewModel::updateProfile) },
                )
            }
