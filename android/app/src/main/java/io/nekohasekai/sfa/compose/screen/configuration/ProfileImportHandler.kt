...
class ProfileImportHandler(private val context: Context) {
    ...
    suspend fun parseQRCode(data: String): QRCodeParseResult = withContext(Dispatchers.IO) {
        try {
            ...
            if (data.startsWith("v2tt://import-remote-profile")) {
                try {
                    val profileInfo = Libbox.parseRemoteProfileImportLink(data.replaceFirst("v2tt://", "sing-box://"))
    ...
    private suspend fun importProfile(content: ProfileContent): ImportResult {
        ...
        io.nekohasekai.sfa.v2tt.ProfileContent.check(content.config)
        io.nekohasekai.sfa.v2tt.ProfileContent.write(configFile, content.config)
    ...
    private suspend fun importRemoteProfile(name: String, url: String): ImportResult {
        ...
        val downloaded = io.nekohasekai.sfa.v2tt.ProfileContent.fetch(url)
        val fileID = ProfileManager.nextFileID()
        val configDirectory = File(context.filesDir, "configs").also { it.mkdirs() }
        val configFile = File(configDirectory, "$fileID.json")
        io.nekohasekai.sfa.v2tt.ProfileContent.write(configFile, downloaded)
        typedProfile.path = configFile.path
    ...
    private fun extractProfileNameFromUrl(url: String): String {
        // Subscription paths contain bearer tokens and are not display names.
        return "V2TT"
    }
    ...
    private fun isJsonConfiguration(content: String): Boolean {
        ...
        return try {
            ...
            json.has("schema_version") || json.has("inbounds") ||
                json.has("outbounds") ||
                json.has("route") ||
                json.has("dns") ||
    ...
    private suspend fun importJsonConfiguration(jsonContent: String, profileName: String): ImportResult {
        return try {
            ...
            try {
                ...
                io.nekohasekai.sfa.v2tt.ProfileContent.check(jsonContent)
            } catch (e: Exception) {
