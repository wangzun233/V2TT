...
import io.nekohasekai.sfa.v2tt.ProfileContent
...
class BoxService(private val service: Service, private val platformInterface: PlatformInterface) : CommandServerHandler {
    ...
    private val accountScope = CoroutineScope(SupervisorJob() + Dispatchers.IO)
    private var accountGuard: Job? = null

    private fun watchAccount() {
        accountGuard?.cancel()
        accountGuard = accountScope.launch {
            while (true) {
                delay(30000)
                val profile = ProfileManager.get(Settings.selectedProfile) ?: continue
                try {
                    ProfileContent.assertUsable(profile)
                } catch (_: Exception) {
                    withContext(kotlinx.coroutines.NonCancellable) {
                        stopAndAlert(Alert.StartService, "账号已到期或订阅需要更新")
                    }
                    return@launch
                }
            }
