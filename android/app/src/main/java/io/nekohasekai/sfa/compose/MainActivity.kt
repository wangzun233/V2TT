...
class MainActivity :
    ...
    private fun handleIntent(intent: Intent?) {
        ...
        if (uri.scheme == "v2tt" && uri.host == "import-remote-profile") {
            try {
                val profile = Libbox.parseRemoteProfileImportLink(uri.buildUpon().scheme("sing-box").build().toString())
    ...
    fun SFAApp() {
        ...
        var showUpdateCheckPrompt by remember {
            mutableStateOf(packageName == "io.nekohasekai.sfa" && !Settings.updateCheckPrompted)
        }
