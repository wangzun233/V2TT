    ...
    class UpdateTask(appContext: Context, params: WorkerParameters) : CoroutineWorker(appContext, params) {
        override suspend fun doWork(): Result {
            ...
            for (profile in remoteProfiles) {
                ...
                try {
                    val changed = ProfileContent.refresh(profile)
                    if (changed && profile.id == selectedProfile) selectedProfileUpdated = true
                } catch (e: Exception) {
                    Log.e(TAG, "Subscription refresh failed (${e.javaClass.simpleName})")
                    success = false
                }
            }
