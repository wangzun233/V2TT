...
android {
    ...
    defaultConfig {
        applicationId = "top.wangzun233.v2tt.android"
        minSdk = 21
        targetSdk = 37
        versionCode = getVersionProps("VERSION_CODE").toInt()
        versionName = getVersionProps("VERSION_NAME")
        testInstrumentationRunner = "androidx.test.runner.AndroidJUnitRunner"
        base.archivesName.set("V2TT-Android-${versionName}")
    }

    signingConfigs {
        create("release") {
            storeFile = file(getProps("V2TT_KEYSTORE_FILE").ifBlank { "v2tt-release-not-configured.keystore" })
